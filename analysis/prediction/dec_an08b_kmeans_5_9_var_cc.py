...
# This scripts will run kmeans clustering on the dataset with complete case on all 11 variables only. 
# load libraries
import pandas as pd 
import numpy as np
import os as os
...
analytic_dataset_cluster['cluster'] = analytic_dataset_cluster['cluster'].replace({0:'MOD', 1:'SIRD', 2:'MARD', 3:'SIDD'})
...
# relabel the cluster labels
data_scaled_cluster['cluster'] = data_scaled_cluster['cluster'].replace({0:'MOD', 1:'SIRD', 2:'MARD', 3:'SIDD'})
...
path_folder = '/Users/zhongyuli/Library/CloudStorage/OneDrive-EmoryUniversity/Diabetes Endotypes Project (JV and ZL)'
contingency_table.to_csv(path_folder + '/working/processed/dec_an08b_contingency_table_clusters_cc.csv')
