...
demographic <- readRDS(paste0(path_endotypes_folder,"/working/interim/dpppre01_demographic.RDS")) %>% 
  distinct(study_id,.keep_all=TRUE)

# There were some cases when the same 'lab_StudyDays' had multiple  values for the same parameter
lab <- readRDS(paste0(path_endotypes_folder,"/working/interim/dpppre02_labs.RDS")) %>% 
...
anthro <- readRDS(paste0(path_endotypes_folder,"/working/interim/dpppre04_anthro.RDS")) %>% 
...
confirmed_dm <- readRDS(paste0(path_endotypes_folder,"/working/interim/dpppre03_events.RDS")) %>% 
  ...
  # Doing these on separate lines because otherwise it will be diabf == 1 AND diagDays == min(diagDays)
  # Rather, we need diabf == 1, and earliest from among those...
