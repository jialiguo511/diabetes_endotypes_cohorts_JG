...
data_path_afu <-  paste0(path_endotypes_folder,"/working/jhs/Data/AFU")
...
#source("functions/convert_formats.R")
...
# convert_formats(data_path_afu,
#                file_name = "afulong.sas7bdat",dest_type = "csv")
...
afu <- data_extract(study_name,vl_column7,data_path_afu) %>% 
  group_by(study_id) %>% 
  summarise(across(
    everything(), 
    ~ case_when(
